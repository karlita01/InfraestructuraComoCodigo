resource "aws_s3_bucket" "frontend" {
  bucket = "minimarket-frontend-charlie-us-east-1"
  force_destroy = true
}

resource "aws_s3_bucket" "frontend_backup" {
  bucket        = "minimarket-frontend-charlie-backup-us-east-1"
  force_destroy = true
}
...
resource "aws_s3_bucket_public_access_block" "frontend_backup_block" {
  bucket                  = aws_s3_bucket.frontend_backup.id
  block_public_acls       = true
  block_public_policy     = true
  restrict_public_buckets = true
...
resource "aws_s3_bucket_policy" "frontend_backup_policy" {
  bucket = aws_s3_bucket.frontend_backup.id
